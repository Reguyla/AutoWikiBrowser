...
public class Article : IProcessArticleEventArgs, IComparable<Article>
{
    ...
    {
        bool noInterwikiChanges;

        AWBChangeArticleText(
            "Fixed interwikis",
            Parsers.InterwikiConversions(mArticleText, out noInterwikiChanges),
            true);

        if (langCode != "en" && langCode != "simple")
            return false;

        string originalText = mArticleText;
        string updatedText = mArticleText;

        if (ShouldApplyGeneralConversions())
        ...
        bool noHeadingChanges;
        updatedText = Parsers.FixHeadings(updatedText, Name, out noHeadingChanges);

        bool anyChanges = !string.Equals(
            originalText,
            updatedText,
    ...
    { // TODO: 2009-01-28 review which of the genfixes below should be labeled 'significant'
        ...
        else
        {
            ...
            // ReorderReferences not for en-wp, uk-wiki or ru-wiki genfixes, T154089, T387784, T418701
            if (!Variables.IsWikipediaEN && Variables.LangCode != "uk" && Variables.LangCode != "ru")
            {
