...
| Analysis Findings | Missing generated `SvnInfo.cs`, unresolved `m_Revision`, unused variable |
...
## 5.4. TODO / FIXME / HACK Inventory / Source forge-GIT drift
## 5.4. Source Annotation Inventory
...
| CA-002 | Medium | `m_Revision` does not exist in the current context during analysis. | Determine if this is generated code or a stale reference. | Open |
| CA-003 | Low | `usingDefaultJSONText` is assigned but never used. | Clean up during refactoring phase. | Deferred |

### 5.4.3 Sourceforge to GITHUB drift
...
## 5.5. Static Analysis (Roslynator / CodeMaid)
...
### 5.5.1. Visual Studio Code Analysis
...
### 5.5.3. Errors/Warnings/Messages baseline
...
### 5.8.1. Partial review of WikiFunctions.Tools
...
### 5.8.2. Partial review of Wikiregexes
#### 5.8.2.1. WikiRegexes Assessment Note
...
### 5.8.3. Review of WikiFunctions.Variables
...
#### 5.8.4.2.  Noting one of the hacks that needs to be addressed
| Area | Finding | Severity | Migration Phase | Status | Recommendation |
|---|---|:---:|:---:|:---:|---|
...
### 5.8.5. Wikiregex URL handler inefficient
| Area | Finding | Severity | Migration Phase | Status | Recommendation |
|---|---|:---:|:---:|:---:|---|
...
### 5.8.7. Visual Studio Analysis errors/Warnings/Messages
#### 5.8.7.1. Errors
...
#### 5.8.7.4. Visual studio test results
| Area | Finding | Severity | Migration Phase | Status | Recommendation |
|---|---|:---:|:---:|:---:|---|
...
### 5.9.1. Test infrastructure Assessment
The test suite is discoverable and executable, but the majority of tests fail in the current local environment because they appear to depend on live Wikipedia/API connectivity or missing test configuration. This limits the immediate usefulness of the test suite as a migration safety net until offline/unit-test isolation is improved.

Tests discovered: 849
Passed: 4
Failed: 845
Likely cause: environment/network dependency
...
#### 5.11.2. as of 20260628
| Metric | Current Result |
|---|---:|
| Build Errors | 0 |
| Build Warnings | 3 |
| Analyzer Messages | 709 |
| Current Commit | 60 |
...
| Area | Finding | Severity | Migration Phase | Status | Recommendation |
|---|---|:---:|:---:|:---:|---|
| Wiki Assumptions | Several features assume English Wikipedia or English-language rules. | Medium | Modernization | Open | Add wiki profile selection and API-based site detection so behavior can be configured per wiki. |
...
### 5.12.1. Coding Style Consistency
